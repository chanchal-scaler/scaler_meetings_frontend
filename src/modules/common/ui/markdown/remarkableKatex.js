...
/**
 * Remarkable Markdown processor which transforms
 * $..$ and $$..$$ sequences into math HTML using the
 * Katex package.
 *
 * This File is copied from this package
 * https://github.com/bradhowes/remarkable-katex
 *
 * Why its copied ?
 * This package has not been maintained for the last 2 years and just to
 * avoid the breakdowns from the new changes copying for safer side.
 */

const remarkableKatex = (md, options) => {
  const backslash = '\\';
  const dollar = '$';
  const opts = options || {};
  ...
  const parseBlockKatex = (state, startLine, endLine) => {
    ...
    /* If a fence has heading spaces, they
    should be removed from its inner block */
  ...
  const parseInlineKatex = (state, silent) => {
    ...
    /**
     * Capture the length of the starting delimiter
     * closing one must match in size
     */
    ...
    while (pos < max) {
      ...
      if (
        char === '{'
        && (pos === 0 || state.src.charAt(pos - 1) !== backslash.charAt(0))
      ) {
        escapedDepth += 1;
      } else if (
        char === '}'
  ...
  md.renderer.rules.katex = (
    tokens, idx,
  ) => renderKatex(tokens[idx].content, tokens[idx].block);
